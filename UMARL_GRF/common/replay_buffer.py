...
class ReplayBuffer:
    def __init__(self, args):
        ...
        if self.args.alg == 'qweight_ac' or self.args.alg == 'qweight_vb':
            self.buffers['adj'] = np.empty([self.size, self.episode_limit, self.n_agents, self.n_agents])
            self.buffers['adj_next'] = np.empty([self.size, self.episode_limit, self.n_agents, self.n_agents])
        # thread lock
    ...
    def store_episode(self, episode_batch):
        ...
        with self.lock:
            ...
            if self.args.alg == 'qweight_ac' or self.args.alg == 'qweight_vb':
                self.buffers['adj'][idxs] = episode_batch['adj']
                self.buffers['adj_next'][idxs] = episode_batch['adj_next']
