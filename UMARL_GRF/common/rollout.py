...
class RolloutWorker:
    ...
    def generate_episode(self, episode_num=None, evaluate=False):
        ...
        if self.args.alg == 'qweight_ac' or self.args.alg == 'qweight_vb':
            Adj, Adj_next = [],[]
        self.env.reset()
        terminated = False
        win_tag = False
        ...
        while not terminated and step < self.episode_limit:
            ...
            if self.args.alg == 'qweight_ac' or self.args.alg == 'qweight_vb':
                Mactrix = self.env.get_visibility_matrix()[:, :self.n_agents]
                for i in range(self.n_agents):
                    Mactrix[i][i] = 1
                Adj.append(Mactrix)
            #print(actions)
            #reward if sum(rewards) <= 0 else self.reward_max, done, infos, sum(rewards) > 0
            reward, terminated, info,_ = self.env.step(actions)
        ...
        for i in range(step, self.episode_limit):
            ...
            terminate.append([1.])
            if self.args.alg == 'qweight_ac' or self.args.alg == 'qweight_vb':
                Mactrix = np.zeros((self.n_agents, self.n_agents)
                                   , dtype=np.bool, )
                Adj.append(Mactrix)
                Adj_next.append(Mactrix)
        ...
        if self.args.alg == 'qweight_ac' or self.args.alg == 'qweight_vb':
            episode['adj'] = np.array([Adj.copy()])
            episode['adj_next'] = np.array([Adj_next.copy()])
